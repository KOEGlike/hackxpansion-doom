...
void *W_CacheLumpNum(int lumpnum, int tag)
{
    ...
    if (lump->wad_file->mapped != NULL
        && (lump->position & 0x80000000u) == 0)
    {
        // Memory mapped file, return from the mmapped region.  Lumps with
        // the top bit set are LZ4-compressed and use the read path instead
        // (xpanse).
