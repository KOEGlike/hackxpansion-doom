...
{
    /* xpanse: the zone lives in a static RAM buffer sized by the build
     * script; there is no heap allocator on the console. */
    extern byte *dg_zone_base(int *size);
    return dg_zone_base(size);
}
